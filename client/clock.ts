...
export function setHourFormat(format: boolean): void {
  hourFormat = format;
}
...
// Day / Night
// -----------------------------
function getDayNight(timezone: string): string {
  const hour = Number(
...
// Compare with India
// -----------------------------
function getDifferenceFromIndia(timezone: string): string {
...
export function createClockCard(
  ...
  const city =
    (timezone.split("/")[1] || timezone).replace(/_/g, " ");
  ...
  const utc = card.querySelector(".utc-offset") as HTMLElement;
  const compare = card.querySelector(".comparison") as HTMLElement;
  const dayNight = card.querySelector(".day-night") as HTMLElement;
